...
export default function ShopPage() {
  ...
  async function fetchSummaryAndDeliveries() {
    ...
    try {
      const data = await fetchWithAuth(`/api/summary/${shopId}`);

    setShopData({
      todayTotal: data?.dailyData?.reduce((sum, item) => sum + item.value, 0),
      transactionsToday: data?.transactionsToday || 0,
      itemsSoldToday: data?.itemsSoldToday || 0,
      weeklyTotal: data?.weeklyData?.reduce((sum, item) => sum + item.value, 0),
      avgBasketValue: data?.avgBasketValue || "0.00",
    ...
    setLowStockProducts(data?.lowStockItems || []);
    ...
    setCategoryData(formattedCategories);
      ...
      // Deliveries
      const deliveries = await fetchWithAuth(`/api/deliveries/${shopId}`);
...
const handleConfirmDelete = async () => {
  ...
  try {
    await fetchWithAuth(`/api/deliveries/${shopId}/${deliveryToDelete._id}`, 'DELETE');
